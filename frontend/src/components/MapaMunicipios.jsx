...
const obtenerMunicipio = (id_municipio, dataMunicipios) => {
  return dataMunicipios.find(
    (municipio) => municipio.id_municipio === id_municipio
  );
};
const calcularColorCoolWarm = (
  id_municipio,
  maximo,
  minimo,
  metricaTmp,
...
const style = (id_estado, maximo, minimo, metricaTmp, dataMunicipios) => {
  return {
    fillColor: calcularColorCoolWarm(
      id_estado,
      maximo,
      minimo,
      metricaTmp,
...
export const MapaMunicipios = () => {
  ...
  const { data, error } = useMunicipiosEstado(fecha, idEstado);
  const {
    data: jsonMapa,
    mapView,
    updatingMapView,
  ...
  if (error) {
    return (
      <div className="text-red-500 text-center p-5">Inténtelo de nuevo</div>
    );
  }
              ...
              return (
                <GeoJSON
                  ...
                  onEachFeature={(feature, layer) => {
                    ...
                    let municipio = obtenerMunicipio(
                      feature.properties.id_municipio,
                      data.data
                    );
                    if (!municipio) {
                      return;
